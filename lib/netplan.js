...
Netplan.prototype.status = async function() {
    const ipResult = await this.executeBinary(this.ipBinary, ['addr']);
    ...
    if (routeResult.stdout) {
        routeResult.stdout.split('\n').forEach(route => {
            if (route.trim().match(/^0.0.0.0/)) {
                ...
                let defaultRoute = route.trim().replace(/\s+/g, ' ').split(' ');
                let gateway = defaultRoute[1];
                let iface = defaultRoute[7];

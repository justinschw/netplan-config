...
Netplan.prototype.configureStaticInterface = function(name, options) {
    const schema = joi.object({
        dhcp: true,
    ...
    let definition = {
        addresses: [`${validated.ip}/${validated.prefix}`],
...
Netplan.prototype.configureDhcpInterface = function(name, options) {
    const schema = joi.object({
        type: joi.string().min(1).valid('ethernet', 'wifi'),
        accessPoint: joi.object({
            ssid: joi.string().min(1).required(),
            wifiPassword: joi.string().min(1).required()
        }).optional()
